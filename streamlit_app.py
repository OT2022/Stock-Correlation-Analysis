...
st.caption("Note: Only UK (.L) and US (.US) tickers are currently supported. More countries to be added soon.")

# User input
ticker = st.text_input("Enter Ticker e.g. VOD.L, BP.L, LLOY.L, AMZN.US", "VOD.L")
...
if ".L" in ticker:
    ...
    st.error("Ticker is not currently available. Please use a ticker ending in '.L' for UK stocks or '.US' for US stocks.")
    st.stop()
